...
const ExtractionSchema = z.object({
    entities: z.array(
        z.object({
            ...
            type: z
                .enum(["Person", "Organization", "Location", "Object", "Concept"])
    ...
    relationships: z.array(
        z.object({
            ...
            type: z
                .string()
                .describe("Relationship type in SCREAMING_SNAKE_CASE, e.g. WORKS_FOR, LOCATED_AT, OWNS"),
    ...
    events: z.array(
        z.object({
            ...
            occurredAt: z
                .string()
                .nullable()
                .describe("ISO8601 datetime string, or null if the date is unknown"),
            confidence: z.number().min(0).max(1).describe("Confidence score from 0.0 to 1.0"),
        }),
...
async function fetchCaseContext(
    caseId: string,
): Promise<{ entities: string; hypotheses: string }> {
    ...
    try {
        ...
        if (entityResult.records.length > 0) {
            existingEntities = entityResult.records
                .map((r) => `- ${r.get("name")} (${r.get("type")}) [id: ${r.get("id")}]`)
                .join("\n");
        }
        ...
        console.warn(
            `[EXTRACTION] ⚠ Failed to fetch case context for caseId=${caseId}:`,
...
export class ExtractionProcessor {
    static async handle(
    ...
    ): Promise<{ evidenceId: string; extractionKey: string; entityCount: number }> {
        const { evidenceId, caseId, normalizedTextKey } = payload;

        console.log(
            `[EXTRACTION] ▶ START evidenceId=${evidenceId} caseId=${caseId} normalizedTextKey=${normalizedTextKey}`,
        );
        ...
        console.log(`[EXTRACTION] First 300 chars: "${text.substring(0, 300)}"`);

        const extractionKey = `cases/${caseId}/extraction/${evidenceId}.json`;

        if (text.trim().length < 100) {
            console.warn(
                `[EXTRACTION] ⚠ Normalized text is very short (${text.length} chars). Uploading empty extraction.`,
            );
            const emptyExtraction = { entities: [], relationships: [], events: [] };
            await StorageService.upload(
                extractionKey,
                Buffer.from(JSON.stringify(emptyExtraction, null, 2)),
                "application/json",
            );
        ...
        console.log(
            `[EXTRACTION] Fetching case context (entities, hypotheses) for caseId=${caseId}...`,
        );
        const { entities: existingEntities, hypotheses: existingHypotheses } =
            await fetchCaseContext(caseId);
        ...
        if (truncated) {
            console.warn(
                `[EXTRACTION] ⚠ Text truncated from ${text.length} to ${MAX_EXTRACTION_CHARS} chars`,
            );
        }

        console.log(
            `[EXTRACTION] Calling LLM for entity/relationship/event extraction (${textForLLM.length} chars)...`,
        );
        let extraction;
        try {
        ...
        console.log(
            `[EXTRACTION] LLM extraction complete — entities=${extraction.entities.length} relationships=${extraction.relationships.length} events=${extraction.events.length}`,
        );

        console.log(`[EXTRACTION] Uploading extraction result to: ${extractionKey}`);
        await StorageService.upload(
            extractionKey,
            Buffer.from(JSON.stringify(extraction, null, 2)),
            "application/json",
        );
        console.log(`[EXTRACTION] Extraction JSON uploaded`);

        console.log(
            `[EXTRACTION] ✓ DONE evidenceId=${evidenceId} entityCount=${extraction.entities.length}`,
        );
